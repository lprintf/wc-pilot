...
function Users({ selected, onSelect }: { selected: number | null; onSelect: (id: number) => void }) {
  ...
  return <section className="panel users"><header className="panel-head"><h1>客服后台</h1><div className="search"><input value={search} onChange={e => setSearch(e.target.value)} placeholder="昵称或内部用户 ID"/><button className="btn" onClick={() => query.refetch()}>刷新</button></div></header><div ref={parent} className="user-scroll">{query.isLoading ? <div className="empty">加载中…</div> : query.error ? <div className="error">{query.error.message}</div> : !users.length ? <div className="empty">暂无用户</div> : <div style={{height: virtual.getTotalSize(),position:"relative"}}>{virtual.getVirtualItems().map(row => { const user=users[row.index]; return <button key={user.id} className={`user-row ${selected===user.id?"active":""}`} style={{position:"absolute",top:0,left:0,width:"100%",height:row.size,transform:`translateY(${row.start}px)`}} onClick={() => onSelect(user.id)}><span className="avatar">{user.nickname.slice(0,1)||"微"}</span><span className="user-copy"><strong>{user.nickname||"未命名用户"}</strong><small>{user.last_message_preview||"暂无消息"}</small></span><span className="user-meta">{formatTime(user.last_active_at)}{user.unread_count>0&&<b>{user.unread_count}</b>}</span></button>})}</div>}</div></section>
}

function Conversation({ userId }: { userId: number | null }) {
  const [content, setContent] = useState("")
  const parent = useRef<HTMLDivElement>(null)
  const queryClient = useQueryClient()
  const query = useInfiniteQuery({ enabled: userId!==null, queryKey: ["conversation", userId], initialPageParam: null as number|null, queryFn: ({pageParam}) => api<{messages: Message[]; has_more: boolean; next_before_id: number|null}>(`/api/admin/users/${userId}/conversation?limit=80${pageParam?`&before_id=${pageParam}`:""}`), getNextPageParam: page => page.has_more ? page.next_before_id : undefined })
